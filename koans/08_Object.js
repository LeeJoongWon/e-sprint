...
*/
    it('Object의 기본을 확인합니다.', function () {
        ...
        expect(megalomaniac.twins['Heath Ledger']).to.deep.equal(
            'The Dark Knight'
        );
    });
    ...
    it("'this'는 method를 호출하는 시점에 결정됩니다.", function () {
        ...
        expect(megalomaniac.calculateAge(currentYear)).to.equal(2022 - 1970);

        megalomaniac.birthYear = 2000;
        expect(megalomaniac.calculateAge(currentYear)).to.equal(2022 - 2000);

        megalomaniac.changeBirthYear(2010);
        expect(megalomaniac.calculateAge(currentYear)).to.equal(2022 - 2010);
